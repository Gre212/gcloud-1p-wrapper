...
```zsh
git clone https://github.com/YOUR_ORG/gcloud-wrapper.git
cd gcloud-wrapper
./install.sh
```
...
Terraform (GCP プロバイダ) を利用する場合は、AWS など他クラウドの作業を意図せず妨害しないよう `terraform-op` という別名コマンドを提供しています。
GCP 専業であれば、`~/.zshrc` に `alias terraform=terraform-op` を設定することで完全に透過的な使い勝手になります。
...
本ツールはローカルの `application_default_credentials.json` を物理的に削除するため、**Python, Go などの Google Cloud Client Libraries (SDK) には非対応です。**
## ロールバック手順
